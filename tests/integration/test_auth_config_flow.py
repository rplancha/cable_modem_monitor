...
from custom_components.cable_modem_monitor.config_flow import _run_auth_discovery_sync
...
class TestSetupDiscoveryNoAuth:
    ...
    def test_setup_discovers_no_auth(self, http_server):
        ...
        result = _run_auth_discovery_sync(
            host=host,
            username=None,
            password=None,
            legacy_ssl=False,
        ...
        assert result["auth_strategy"] == AuthStrategyType.NO_AUTH.value
...
class TestSetupDiscoveryBasicAuth:
    ...
    def test_setup_discovers_basic_auth(self, basic_auth_server):
        ...
        assert result["auth_strategy"] == AuthStrategyType.BASIC_HTTP.value
        assert result["auth_discovery_status"] == "success"
        assert result["auth_discovery_error"] is None
    ...
    def test_basic_auth_wrong_credentials(self, basic_auth_server):
        ...
        assert result["auth_discovery_error"] is not None
...
class TestSetupDiscoveryFormAuth:
    ...
    def test_form_auth_stores_form_config(self, form_auth_server):
        ...
        form_config = result.get("auth_form_config")
...
class TestSetupDiscoveryRedirect:
    ...
    def test_setup_follows_meta_refresh_redirect(self, redirect_auth_server):
        ...
        assert result["auth_discovery_status"] in ("success", "error")
        # Form should be detected even after redirect
        if result["auth_discovery_status"] == "success":
            assert result["auth_form_config"] is not None
...
class TestSetupDiscoveryHNAP:
    ...
    def test_setup_detects_hnap(self, hnap_auth_server):
        ...
        assert result["auth_strategy"] == AuthStrategyType.HNAP_SESSION.value
        assert result["auth_discovery_status"] == "success"
...
class TestConfigEntryIntegration:
    ...
    def test_strategy_is_string(self, http_server):
        ...
        assert isinstance(result["auth_strategy"], str)
    ...
    def test_result_keys_match_constants(self, http_server):
        """Result keys match the CONF_AUTH_* constants."""
        host = http_server.url
        ...
        # These keys should match the constants in const.py
        assert "auth_strategy" in result
        assert "auth_form_config" in result
        assert "auth_discovery_status" in result
        assert "auth_discovery_error" in result
        assert "authenticated_session" in result
        assert "authenticated_html" in result
